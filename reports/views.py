...
def total_leave_applications():
    ...
    total_applications = LeaveApplication.objects.aggregate(
        ...
        today=models.Count('id', filter=models.Q(
...
def due_resumptions():
    _resumption_status = ['head of unit',
                          'head of department', 'directorate', 'none']
    now = timezone.now()
...
def active_leave_list_view(request):
    status = ['partly active', 'active', 'done',
              'partly done', 'resuming', 'partly resuming']
    active_apps = None
